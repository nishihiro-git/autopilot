...
import { LightbulbIcon, SaveIcon, PaletteIcon, CheckIcon, SparklesIcon, Loader2Icon, ImageIcon } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";

interface DesignInstructionsProps {
  initialInstructions?: string;
  onInstructionsChange?: (instructions: string) => void;
}

export default function DesignInstructions({
  initialInstructions = "",
  onInstructionsChange,
}: DesignInstructionsProps) {
  ...
  const exampleInstructions = [
    "モダンでミニマルなスタイル。白背景に黒文字、アクセントカラーは#3498db。",
    "未来的でサイバーパンク風。ネオンカラーを多用し、グリッチエフェクトも少し。",
    "暖かみのある手書き風イラスト。パステルカラーを基調に。",
    "高級感のあるダークテーマ。ゴールドをアクセントに、エレガントなフォントを使用。",
  ];
  ...
  const generateImage = async () => {
    if (!instructions.trim()) {
      toast({
        title: "デザイン指示が必要です",
        description: "画像生成するデザイン指示を先に入力してください。",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const response = await fetch("/api/ai/generate", {
        ...
        body: JSON.stringify({
          type: "image",
          keywords: ["design", "visual"], // デザイン用のキーワード
          design: instructions,
        }),
      });
      ...
      if (response.ok) {
        ...
        toast({
          title: "画像生成完了",
          description: "デザイン指示に基づいて画像を生成しました。",
        });
        console.log("生成された画像:", result.data);
      } else {
    ...
    <div className="space-y-6 sm:space-y-8 p-4 sm:p-8 bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl shadow-2xl shadow-slate-950/50">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
        <div className="flex items-center gap-2 sm:gap-3">
          <PaletteIcon className="h-5 w-5 sm:h-6 sm:w-6 text-yellow-400" />
          <h3 className="text-xl sm:text-2xl font-bold text-slate-50">デザイン指示</h3>
        </div>
        <Button
          onClick={generateImage}
          disabled={isGenerating || !instructions.trim()}
          className="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white font-bold py-2 px-3 sm:px-4 text-xs sm:text-sm rounded-md transition-all duration-200 hover:scale-105 disabled:scale-100 disabled:opacity-50 w-full sm:w-auto"
        >
          {isGenerating ? (
      ...
      <div className="space-y-3 sm:space-y-4">
        <textarea
          ...
          placeholder="デザインの指示を入力してください（例：モダンでミニマルなデザイン、青と白の配色、シンプルなアイコン）"
        ...
        {generatedImage && (
          <div className="space-y-2 sm:space-y-3">
            <h4 className="text-base sm:text-lg font-semibold text-slate-200">生成された画像</h4>
            <div className="relative">
              <img
                src={generatedImage}
                alt="AI生成画像"
                className="w-full h-32 sm:h-48 object-cover rounded-lg border border-slate-600"
              />
              <div className="absolute top-2 right-2 bg-slate-900/80 text-slate-300 text-xs px-2 py-1 rounded">
                AI生成
              </div>
            </div>
          </div>
      ...
      <div className="text-xs sm:text-sm text-slate-400 space-y-1 sm:space-y-0">
        <p>• デザインのスタイル、色合い、レイアウトなどを具体的に指示してください</p>
        <p>• 例：「モダンでミニマル」「カラフルでポップ」「エレガントで洗練された」</p>
        <p>• AI画像生成ボタンで、指示に基づいて画像を自動生成できます</p>
      </div>
    </div>
  );
}
